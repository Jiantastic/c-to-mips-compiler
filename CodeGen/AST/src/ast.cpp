...
void ShuntingYardAlgo(std::vector<Expression*> &completeTree,std::stack<int> &mystack,const bool &debugMode,mipsRegisters &mips32,std::string declarator="",std::string assignOp=""){
	...
	if(declarator == ""){
		...
		mips32.Bind(final_answer,v,declarator);
		codeGen(v,mips32);
	}
	completeTree.clear();
	mystack.pop();
